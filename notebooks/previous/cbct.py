...
def show_ellipses(e2, cog_allbeads):
    ...
    for idx in range(len(e2)):
        ...
        ell_patch = Ellipse((e2[idx][0], e2[idx][1]), 2 * e2[idx][2], 2 * e2[idx][3], theta, edgecolor='red',
                            facecolor='none')
...
def estimate_sod_sdd(tcal, e2, vpiercing, mag, factor=1.08):
    ...
    for idx in range(np.array(e2).shape[0]):
        ...
        print("h: {0:0.3f}, SOD: {1:0.2f}, SDD: {2:0.2f}, magn: {3:0.2f}, ha: {4:0.2f}, hb: {5:0.2f}".format(h, est_sod,
                                                                                                             est_sdd,
                                                                                                             est_sdd / est_sod,
                                                                                                             ha, hb))
    sd_sod = np.std(sod)
