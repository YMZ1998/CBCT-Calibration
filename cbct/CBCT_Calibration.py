...
class CBCTCalibration:
    ...
    def set_projections(self, proj, ob=None, dc=None, verticalflip=False, show=False, amplification=10, stack_window=5):
        start_time = time.time()
        proj = proj.astype(np.float32)

        if ob is None or dc is None:
            dc_scalar = np.min(proj)  # 全局最小值
            ob_scalar = 30000  # 预设最大值
            print("dc: %f, ob: %f" % (dc_scalar, ob_scalar))
            ob_scalar = max(ob_scalar - dc_scalar, 1.0)

            dc = np.full(proj.shape[1:], dc_scalar, dtype=np.float32)  # shape: (H, W)
            ob = np.full(proj.shape[1:], ob_scalar, dtype=np.float32)  # shape: (H, W)
        else:
            ob = ob - dc
            ob[ob <= 0] = 1
        ...
        # 矢量化归一化操作
        # tmp = (projections - dc), 小于等于0的置1，再除以ob
        tmp = self.projections - dc[None, :, :]
        tmp[tmp <= 0] = 1
        self.projections = tmp / ob[None, :, :]

        # 限制最大值为1.5
        np.clip(self.projections, None, 1.5, out=self.projections)

        if verticalflip:
            self.projections = self.projections[:, ::-1, :]
        end_time = time.time()
        print(f"set_projections 耗时: {end_time - start_time:.4f} 秒")
        if show:
    ...
    def remove_projection_baseline(self, show=False):
        ...
        b2 = np.median(self.projections, axis=0)
        self.projections_flat = b2[None, :, :] - self.projections
        np.clip(self.projections_flat, 0, None, out=self.projections_flat)
        self.histogram = np.histogram(self.projections_flat, bins=1000)

        end_time = time.time()
        print(f"remove_projection_baseline 耗时: {end_time - start_time:.4f} 秒")
    ...
    def flatten_projections(self, amplification=1, stack_window=0, show=False):
        ...
        self.histogram = np.histogram(self.projections_flat.flatten(), bins=1000)
        end_time = time.time()
        print(f"flatten_projections 耗时: {end_time - start_time:.4f} 秒")
        if show:
            _, ax = plt.subplots(1, 2, figsize=(10, 5))
            ax[0].imshow(self.projections_flat[0], vmin=-0.1, vmax=1)
            ax[1].plot(self.histogram[1][0:-1], self.histogram[0])
    ...
    def  find_beads(self, min_distance=5, breakup=False, breakup_threshold=1, show=False):
        ...
        for (idx, (biproj, flat)) in enumerate(zip(self.projections_bilevel, self.projections_flat)):
            ...
            for region in rp:
                if region.area > 20:
                    self.beads.append({"idx": idx, "label": region.label, "centroid": np.array(
                        [region.centroid_weighted[0], region.centroid_weighted[1]])})
    ...
    def _prune_ellipses(self):
        ...
        self.ellipses = [ellipse for ellipse in self.ellipses if
                         ellipse["ellipse"]["width"] < mean_major + 2 * std_major]
    ...
    def show_ellipses(self):
        ...
        for idx, ellipse in enumerate(self.ellipses):
            e = ellipse['ellipse']
            if e is not None:
                ellipse_patch = Ellipse(xy=e["xy"], width=e["width"], height=e["height"], angle=e["angle"],
    ...
    def compute_calibration(self, diameter=50, pixelsize=0.139, avgtype='median', remove_outliers=True, ppdegree=3,
                            show=False):
        """
    ...
    def _compute_piercing_point(self):
        ...
        for idx in range(3):
            ...
            r = r0 + r1
            print("r0:", r0, "shape:", r0.shape)
            print("r1:", r1, "shape:", r1.shape)
    ...
    def _compute_piercing_point2(self, degree=5, show=False):
        ...
        segment_lengths = np.sqrt(
            (line_points[:, 2] - line_points[:, 0]) ** 2 + (line_points[:, 3] - line_points[:, 1]) ** 2)
        ...
        params, params_covariance = curve_fit(lambda xy, *params: _pp_higher_order_model(xy, *params), X.T, Y,
                                              p0=initial_guess)
        ...
        result = minimize(_pp_objective, x0=[(X[:, 0].min() + X[:, 0].max()) / 2, (X[:, 1].min() + X[:, 1].max()) / 2],
                          bounds=bounds)
    ...
    def show_calibration(self):
        ...
        ax[1].axhline(y=self.calibration["pp"]["y"], color='r',
                      label="py={0:0.02f}".format(self.calibration['pp']['y']))
